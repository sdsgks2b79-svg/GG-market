...
bot.on("text", async ctx=>{
  ...
  if(ctx.session.pending){
    ...
    const cleaned = txt.replace(",",".").replace(/[^0-9.,a-zA-Zа-яА-ЯёЁғҒқҚўЎҳҲ]+/g,"");
    ...
    if(product.unit==="piece"){
      ...
      // Agar matnda "kg" bo'lsa → kg bo'yicha olish
      const isKg = lower.includes("kg") || lower.includes("кг");

      // Pulga oid so'zlar (som, so'm, sum, kirill, variantlar)
      const moneyWords = ["so'm", "som", "sum", "сом", "сум", "сўм", "so", "so'mlik", "somlik", "sumlik"];

      const isMoney = moneyWords.some(w => lower.includes(w));
      ...
      if(isKg){
          ...
          // kg YO'Q bo'lsa → avtomatik so'mlik deb qabul qilamiz
...
// 🚫 Yetkazib berish vaqtincha ishlamaydi
bot.action("choose_delivery_delivery", async ctx=>{
  await ctx.answerCbQuery();
  await ctx.reply("Hozircha yetkazib berish ishlamayapti, tez orada ishga tushirib sizga xabar beramiz! 😊");
});
...
async function finalizeOrder(ctx,paymentMethod){
  ...
  try{
    ...
    // ✔ mijozga to‘g‘ri xabar chiqaramiz
    let clientMsg = "";
    if (deliveryType === "Olib ketish") {
      clientMsg = "Buyurtmalar tayyor, olib ketishingiz mumkin! 😊";
    } else {
      clientMsg = "Buyurtmangiz qabul qilindi. Tez orada yetkazib beramiz. 🚚";
    }

    await ctx.replyWithDocument({source:pdfPath, filename:path.basename(pdfPath)});
    await ctx.reply("✅ " + clientMsg, mainMenuKeyboard());
    ...
    console.error(err);
    ctx.reply("Buyurtma yaratilishda xatolik yuz berdi.");
  }
}
