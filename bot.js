...
function categoriesKeyboard() {
  const buttons = CATEGORIES.map((c, i) => Markup.button.callback(`${CATEGORY_EMOJIS[c]||"📦"} ${c}`, `cat_${i}`));
...
bot.action(/^cat_(\d+)$/, async ctx => {
  ...
  const cat = CATEGORIES[idx];
  await ctx.reply(`📦 ${cat}`, productsKeyboard(idx));
});
...
bot.on("text", async ctx => {
  ...
  if(ctx.session.pending) {
    ...
    if(!product) return ctx.reply("Mahsulot topilmadi.");
    let num = parseFloat(txt.replace(",", ".").replace(/[^0-9.]/g,""));
    ...
    ctx.session.cart.push({ productId:product.id,name:product.name,unit:unitType,unitPrice:product.price,quantity,price });
...
// photo from client -> admin
bot.on('photo', async ctx=>{
  await bot.telegram.sendPhoto(ADMIN_ID, ctx.message.photo[ctx.message.photo.length-1].file_id,{
    caption:`📸 Mijoz to'lov skrenshoti\nTelefon: ${ctx.session.phone||"Noma'lum"}`
  });
  await ctx.reply("Skrenshot qabul qilindi ✅ Tez orada tekshirib tasdiqlaymiz.");
});
