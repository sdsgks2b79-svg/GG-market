...
// /start
bot.start(async (ctx) => {
  await ctx.reply("Assalomu alaykum! Telefon raqamingizni yuboring.", mainMenuKeyboard());
});
...
bot.action("ai_mode", async (ctx) => {
  await ctx.answerCbQuery();
  return ctx.reply("Tabiiy tilda yoki shevada yozing: '2ta pepsi va 0.5 kg kartoshka qo‘sh'. AI xato yoki shevada yozilganini ham tushunadi.");
});
