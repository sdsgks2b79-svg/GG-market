...
async function loadProducts() {
  ...
  if (error) { console.error(error); PRODUCTS=[]; CATEGORIES=[]; return; }
  PRODUCTS = data.map(p => ({
    id: Number(p.id),
    name: p.name,
...
bot.on("contact", async ctx=>{
  ...
  if(phone){ ctx.session.phone = phone; await ctx.reply(`Telefon saqlandi: ${phone}`, mainMenuKeyboard()); }
  else await ctx.reply("Kontakt topilmadi, iltimos qayta yuboring.");
});

// Mahsulotlar
bot.hears("🍏 Mahsulotlar", async ctx=>{
  if(!CATEGORIES.length){ await ctx.reply("Hozirda mahsulotlar mavjud emas."); return; }
...
bot.action(/^product_(\d+)$/, async ctx=>{
  ...
  return ctx.reply(`Necha ${product.unit==="kg"?"kilogram":"so'm"} olasiz? (masalan: 0.5 yoki 2500)`);
});
...
bot.on("text", async ctx=>{
  ...
  if(isNaN(num)||num<=0) return ctx.reply("Iltimos to'g'ri son kiriting (1, 0.5, 2500).");
  ...
  if(pending.unit==="piece"){ quantity=Math.round(num); price=product.price*quantity; unitType="piece"; }
    ...
    if(num<product.price && !txt.includes(".") && !txt.includes(",")){
      const sum=Math.round(num); quantity=+(sum/product.price); price=sum; unitType="sum";
...
// --- Checkout, delivery, payment, finalizeOrder ---
// shu yerda eski koddagi checkout, askPaymentMethod, pay_cash/pay_click, finalizeOrder kodlarini to'liq qo'shish mumkin

// Launch
bot.launch().then(()=>console.log("Bot ishga tushdi 🚀")).catch(e=>console.error(e));
