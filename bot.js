...
bot.action(/^product_(\d+)$/, async ctx => {
  ...
  if (product.unit === "piece") {
    ...
    return ctx.reply("Nechta olasiz yoki necha so'mlik olasiz?");
  }
});
...
bot.on("text", async ctx => {
  ...
  if (pending.unit === "piece") {
    ...
    if (num < product.price) { // pul kiritilgan bo‘lishi mumkin
...
// Savat, checkout, delivery, payment va finalizeOrder kodlari xuddi avvalgidek ishlaydi
// (to‘liq yuqoridagi kod bilan birlashtiriladi)
