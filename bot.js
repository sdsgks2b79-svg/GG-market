...
bot.action(/prod_(\d+)/, async (ctx) => {
  ...
  if (!product) {
    return ctx.answerCbQuery("Xatolik: mahsulot topilmadi!", {
      show_alert: true
    });
  }
...
bot.on("location", async (ctx) => {
  ...
  // Do‘kon lokatsiyasi
  const shop =
    "https://maps.app.goo.gl/CmNSNouqpqDdZS6X7?g_st=ic";

  ctx.reply(
    "📦 Buyurtmangiz qabul qilindi!\n" +
      "🏪 Mana do‘konimiz manzili:\n" +
      shop
  );
