...
function cartSummary(session) {
  ...
  const lines = cart.map(ci => {
    ...
    else if (ci.unit === "kg" || ci.unit === "sum") return `• ${ci.name} — ${ci.unit === "kg"? Number(ci.quantity).toFixed(2) + " kg" : formatCurrency(ci.quantity) + " so'm"} × ${formatCurrency(ci.unitPrice)} = ${formatCurrency(ci.price)} so'm`;
    else return `• ${ci.name} — ${formatCurrency(ci.price)} so'm`;
  });
  return { lines, total };
}
...
bot.on("contact", async ctx => {
  ...
  } else ctx.reply("Kontakt topilmadi, iltimos qayta yuboring.");
});
...
bot.action(/^product_(\d+)$/, async ctx => {
  ...
  if (product.unit === "piece") return ctx.reply("Nechta dona olasiz?");
...
bot.on("text", async ctx => {
    ...
    switch (txt) {
      case "🛒 Savatim":
        ...
        await ctx.reply(`🛍 Sizning savatchangiz:\n\n${lines.join("\n")}\n\nJami: ${formatCurrency(total)} so'm`,
          Markup.inlineKeyboard([[Markup.button.callback("✅ Buyurtmani tasdiqlash","start_checkout"), Markup.button.callback("🗑️ Savatni tozalash","clear_cart")]]));
        return;
      case "📞 Sotuvchi bilan bog'lanish": return ctx.reply("Sotuvchi: +998200012560");
      case "📍 Do'kon manzili": return ctx.reply("Do'kon manzili: https://maps.app.goo.gl/8VjBiyPwPGP7nHZZ6?g_st=ic");
      case "🎁 Maxsus takliflar": return ctx.reply("Hozircha hech narsa yo'q 😊");
      case "💳 Qarzlarim": return ctx.reply("Hozircha qarzingiz yo'q ✅");
      default: return ctx.reply("Menyudan tanlang yoki mahsulot miqdorini kiriting.");
    }
  }
  ...
  if (pending.unit === "piece") {
    ...
    if (num < product.price && !txt.includes(".") && !txt.includes(",")) {
      quantity = +(num / product.price); price=num; unitType="sum";
    } else { quantity=num; price=Math.round(product.price*num); unitType="kg"; }
  }
  ...
  await ctx.reply(`${product.name} savatchaga qo'shildi ✅\n\n${lines.join("\n")}\n\nJami: ${formatCurrency(total)} so'm`,
    Markup.inlineKeyboard([[Markup.button.callback("🛒 Savatni ko'rish","show_cart"), Markup.button.callback("✅ Buyurtmani tasdiqlash","start_checkout")]]));
});

// Cart, checkout, delivery, payment, finalize same as old code...
