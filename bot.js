...
bot.hears("🔍 Qidiruv", async ctx=>{
  ...
  await ctx.reply("Nimani qidiryapsiz? Iltimos mahsulot nomini yozing:");
});
...
bot.on("text", async ctx=>{
  ...
  if(ctx.session.awaitingSearch){
    ...
    if(!results.length){
      ctx.reply("Hech qanday mahsulot topilmadi 😔");
    } else {
      ...
      buttons.push(Markup.button.callback("🔙 Ortga", `back_main`));
      ctx.reply("Topilgan mahsulotlar:", Markup.inlineKeyboard(chunkButtons(buttons,1)));
    }

    ctx.session.awaitingSearch = false;
    return; // Qidiruv tugadi
  }

  // Shu yerda boshqa text handler kodlari: Savat, Buyurtma, Maxsus takliflar, miqdor kiritish
  // switch(txt){ case "🛒 Savatim": ... }
});
...
// Qolgan checkout, cart, PDF, admin xabarlar kodi o‘zgarmasdan qoladi
