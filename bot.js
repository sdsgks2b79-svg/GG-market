...
function productsKeyboardForCategoryIndex(idx) {
  ...
  const productButtons = prods.map(p =>
    Markup.button.callback(
      `${p.emoji} ${p.name} — ${formatCurrency(p.price)}${p.unit==="kg"? " so'm/kg":" so'm"}`,
      `product_${p.id}`
    )
  ).map(b => [b]); // har biri alohida qator

  // tagida savat va buyurtma tasdiqlash
  productButtons.push([
...
bot.on("text", async ctx => {
  ...
  await ctx.reply(`${product.name} savatchaga qo‘shildi ✅\n\n${lines.join("\n")}\n\nJami: ${formatCurrency(total)} so'm\n\nYana boshqa mahsulotlarni tanlang👇🏻`, productsKeyboardForCategoryIndex(CATEGORIES.indexOf(product.category)));
});
...
bot.action("start_checkout", async ctx => {
  ...
  const { lines, total } = cartSummary(ctx.session);
  if (!lines.length) return ctx.reply("Savatcha bo‘sh!");
  await ctx.reply(`To‘lov uchun karta raqam: ${CARD_NUMBER}\nTo‘lovni amalga oshirib skreenshotini menga yuboring`);
});

// PDF va tasdiqlashni admin ko‘radi
bot.on("photo", async ctx => {
  ...
  // adminga yuborish
  await bot.telegram.sendPhoto(ADMIN_ID, file_id, { caption: `📌 Yangi to‘lov skrenshoti` });
  await ctx.reply("Skrenshot qabul qilindi. Buyurtmangiz yig‘ilmoqda ✅");
});

// Admin manual reload products
