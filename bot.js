...
bot.action(/^product_(\d+)$/, async ctx => {
  ...
  if (product.unit === "kg") return ctx.reply(`Necha kilogram yoki necha so'mlik olasiz?`);
});
...
bot.on("text", async ctx => {
  ...
  // avtomatik keyingi xaridlar uchun menyuni ko'rsatish
  await ctx.reply("Bo'limni tanlang:", categoriesKeyboard());
});
...
bot.action("pay_click", async ctx => { 
  ...
  if (CLICK_PAY_URL) await ctx.reply(`Click orqali to'lash: ${CLICK_PAY_URL}${total}`);
...
async function finalizeOrder(ctx, paymentMethod) {
  ...
  } catch(err){ console.error(err); return ctx.reply("Buyurtma yaratilishda xatolik yuz berdi."); }
}

// Admin
bot.command("reload_products", async ctx => {
  if (String(ctx.from.id)!==String(ADMIN_ID)) return ctx.reply("Faqat admin.");
  await loadProducts(); return ctx.reply("Products qayta yuklandi.");
});
