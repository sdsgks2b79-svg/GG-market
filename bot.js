...
const STORE_LOCATION = "🏬 Do‘kon manzili: https://maps.app.goo.gl/8VjBiyPwPGP7nHZZ6?g_st=ic";
const CONTACT_PHONE = "200012560";
...
let PRODUCTS = [];
let CATEGORIES = [];
const USER_DEBTS = {}; // Foydalanuvchi qarzlarini saqlash
const SPECIAL_OFFERS = []; // Maxsus takliflar
...
function productsKeyboardForCategoryIndex(idx) {
  ...
  ).map(btn => [btn]); // Har biri alohida qator
...
bot.on("text", async ctx => {
  ...
  let txt = ctx.message.text.trim().replace(/,/g, '.').replace(/[^0-9.]/g, '');
  ...
  await ctx.reply(`${product.name} savatchaga qo‘shildi ✅\n\n${lines.join("\n")}\n\nJami: ${formatCurrency(total)} so'm\n\nHaridlarni davom ettiring👇🏻`, productsKeyboardForCategoryIndex(CATEGORIES.indexOf(product.category)));
});
...
bot.on("location", async ctx => {
  ensureSession(ctx);
  if (ctx.session.checkoutType !== "delivery") return;
  ...
  const pdfPath = await createOrderPdf({
    ...
    address: `Lat: ${ctx.session.location.latitude}, Lon: ${ctx.session.location.longitude}`
  });

  await ctx.reply(`To‘lov uchun karta raqam: ${CARD_NUMBER}\nTo‘lovni amalga oshirib skreenshotini menga yuboring`, Markup.inlineKeyboard([
...
bot.on("photo", async ctx => {
  ...
  await ctx.telegram.sendPhoto(ADMIN_ID, photoId, { caption: `📌 To‘lov screenshoti: ${ctx.from.username || ctx.from.first_name}` });
  await ctx.reply("To‘lov screenshot qabul qilindi, buyurtmangiz tasdiqlanadi ✅");
});

// Confirm payment
bot.action("confirm_payment", async ctx => {
  await ctx.answerCbQuery();
  await ctx.reply("Buyurtmangiz yig‘ilyapti, tez orada tasdiqlanadi ✅");
  await sendOrderToAdmin(ctx);
  ctx.session.cart = [];
...
async function sendOrderToAdmin(ctx) {
  ...
  const pdfPath = await createOrderPdf({
    ...
    lines,
    total,
    deliveryType: ctx.session.checkoutType === "delivery" ? "Yetkazib berish" : "Olib ketish",
    address: ctx.session.checkoutType === "delivery" ? `Lat: ${ctx.session.location.latitude}, Lon: ${ctx.session.location.longitude}` : STORE_LOCATION
  });
  await ctx.telegram.sendDocument(ADMIN_ID, { source: pdfPath, filename: path.basename(pdfPath) }, {
    caption: `📦 Yangi buyurtma: ${ctx.from.username || ctx.from.first_name}`
  });
}

// Qarzlar
bot.hears("💳 Qarzlarim", async ctx => {
  const debt = USER_DEBTS[ctx.from.id] || 0;
  if (debt > 0) return ctx.reply(`Sizning qarzingiz: ${formatCurrency(debt)} so'm`);
...
bot.hears("📍 Do'kon manzili", async ctx => {
  return ctx.reply(`${STORE_LOCATION}`);
});

// Sotuvchi bilan bog‘lanish
bot.hears("📞 Sotuvchi bilan bog'lanish", async ctx => {
  return ctx.reply(`📱 Telefon raqam: ${CONTACT_PHONE}`);
});
