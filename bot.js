...
if (!BOT_TOKEN || !SUPABASE_URL || !SUPABASE_KEY || !ADMIN_ID) {
  throw new Error(".env faylda BOT_TOKEN, SUPABASE_URL, SUPABASE_KEY va ADMIN_ID bo‘lishi shart");
}
...
bot.on("contact", async ctx => {
  ...
  if (phone) {
    ...
    await ctx.reply("Kontakt topilmadi, qayta yuboring.");
  }
});
...
bot.action(/^product_(\d+)$/, async ctx => {
  ...
  if (product.unit === "piece") {
    return ctx.reply(`Nechta olasiz? (butun dona)`);
  } else { // kg
...
bot.action("pay_click", async ctx=>{
  ...
  await ctx.reply(`Click orqali to‘lash (demo) — summa: ${formatCurrency(total)} so'm`);
...
async function finalizeOrder(ctx,paymentMethod){
  ...
  try{
    ...
    await ctx.reply("✅ Buyurtmangiz qabul qilindi. /start", mainMenuKeyboard());
  ...
  }catch(err){console.error(err); await ctx.reply("Buyurtma yaratilishda xatolik yuz berdi.");}
}
